...
public class GameState extends State implements Runnable{
	...
	private int[] dx = {-50, 1200, -100, 900, -360, 1000},
				  dy = {445, 260, 128, 482, 360, 160};
	public static int target, score;
...
//		timer = 41*60;
	...
	public void tick() {
		...
		if(timer <= 0 && score >= target) {
			timer = 0;
			won = true;
			game.setGameOverState(new GameOverState(game, 1));
			State.setState(game.getGameOverState());
		} else if(timer <= 0 && score < target) {
			timer = 0;
			won = false;
			game.setGameOverState(new GameOverState(game, 1));
			State.setState(game.getGameOverState());
	...
	public void render(Graphics g) {
		...
		g.drawString("Score: " + score, 45, 24);
		g.drawString("Target: " + target, 378, 24);
